...
public class JapaneseTLB implements TrafficLightStateBehaviour{

    // The traffic light must be horizontally placed, not vertically
    ...
    //Traffic Light - State Go - Blue Light - needs to be 6s
    @Override
    public void stateGo(Context ctx) {
        ctx.getCurrentState().setColours(List.of(Colour.BLUE));
        ctx.getCurrentState().setTimer(2000);
    }
